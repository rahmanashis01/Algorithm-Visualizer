    ...
    <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
      {algorithms.map((algorithm) => (
        <Link key={algorithm.id} href={`/${algorithm.id}`} className="block group">
          <Card className="algorithm-card-glass overflow-hidden transition-shadow hover:shadow-lg h-full flex flex-col">
            ...
            <CardContent className="flex-grow flex flex-col justify-between">
              <p className="text-lg text-white">{algorithm.description}</p>
            </CardContent>
          </Card>
        </Link>
      ))}
